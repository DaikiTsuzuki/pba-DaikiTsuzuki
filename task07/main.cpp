...
void solve_laplace_gauss_sidel_on_grid(
  ...
  for (unsigned int ix = 0; ix < grid_size; ++ix) {
    for (unsigned int iy = 0; iy < grid_size; ++iy) {
	  ...
	  float sum_neighbors = vtx2val[idx_center - 1] + vtx2val[idx_center + 1] + vtx2val[idx_center - grid_size] + vtx2val[idx_center + grid_size];
	  int count_neighbors = 4;

	  vtx2val[idx_center] = sum_neighbors / count_neighbors;
    }
  }
}
